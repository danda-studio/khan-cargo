...
export function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <Menu.Root modal={false}>
      <Menu.Trigger
        className={cn(
          "flex h-[2rem] w-fit cursor-pointer items-center gap-[0.375rem] rounded-[0.3333rem] border border-white/20 text-[0.875rem] font-medium tracking-[-0.04rem] text-white outline-none transition-colors data-[popup-open]:border-white/40 lg:h-[3.3333rem] lg:gap-[0.6667rem] lg:text-[1.3333rem]",
          className,
        )}
      >
        <img src="/images/arrow-bottom.svg" alt="" className="ml-[0.5rem] size-[1rem] lg:ml-[0.6875rem] lg:size-[1.6667rem]" />
        <span className="mr-[0.5rem] lg:mr-[0.6875rem]">{locale.toUpperCase()}</span>
      </Menu.Trigger>
      <Menu.Portal>
