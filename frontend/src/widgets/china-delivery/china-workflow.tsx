...
export function ChinaWorkflow({ variant = "china" }: { variant?: RouteDeliveryVariant }) {
  ...
  return (
    <section className="overflow-x-clip bg-black pt-[3.25rem] pb-[3.25rem] md:pt-[3.6875rem] md:pb-[5rem]">
      <div className="flex flex-col gap-[2.5rem]">
        <div className={`flex flex-col gap-[1rem] ${chinaPad}`}>
          ...
          <h2 className="text-[1.5rem] leading-[1.1] tracking-[-0.0125rem] text-white md:text-[clamp(1.5rem,2.25vw,2.25rem)]">
            {t.workflow.title}
          </h2>
        </div>

        <div className="scrollbar-hide flex gap-[0.5rem] overflow-x-auto px-[1rem] md:overflow-visible md:px-[2.25rem]">
          {t.workflow.steps.map((step, index) => (
            <div
              key={step}
              className="flex h-[12rem] w-[17.5rem] shrink-0 flex-col justify-between rounded-[0.375rem] bg-[#151515] p-[1.25rem] md:h-auto md:min-h-[12rem] md:w-auto md:min-w-0 md:flex-1"
            >
              <img src={icons[index] ?? icons[0]} alt="" className="size-[3rem]" />
              <p className="text-[1.125rem] leading-[1.2] text-white md:text-[1.25rem]">{step}</p>
            </div>
          ))}
        </div>
